...
namespace ORB {
    ...
    void ORBFeature::ExtractORB()
    {
        ...
        cv::Ptr<cv::ORB> detector = cv::ORB::create(camera_ptr->nFeatures, camera_ptr->scalseFactor, camera_ptr->nLevels);
        std::vector<cv::KeyPoint> vKeypoints;
        detector->detect(image, vKeypoints, cv:: noArray());
        cv::Mat outImage2;
        cv::drawKeypoints(image, vKeypoints, outImage2, cv::Scalar::all(-1), cv::DrawMatchesFlags::DEFAULT);
        cv::imshow("这是OpenCV的提取结果", outImage2);
    }
    ...
    void ORBFeature::ExtractORB(std::vector<cv::KeyPoint>& _keypoints, cv::Mat& _descriptors)
    {
        ...
        for (int level = 0; level < camera_ptr->nLevels; ++level)
        {
            ...
            GaussianBlur(workingMat,workingMat,cv::Size(7, 7),
